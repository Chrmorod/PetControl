...
int led(String command) {
    ...
    digitalWrite(ledPin, ledState ? HIGH : LOW); // Establece el pin del LED
    return ledState ? 1 : 0; // Devuelve el estado del LED (1 para encendido, 0 para apagado)
}
float getWeightGrams(String command) {
    scale.set_scale(calibration_factor); 
    float weight = abs(scale.get_units(50)); 
...
void setup() {
    ...
    Particle.function("led", led); // Registra la función remota
    Particle.function("adjustCalibration", adjustCalibrationFactor);
    Particle.function("getWeightGrams",getWeightGrams);
    ...
    notify("Ajuste el factor de calibración con los comandos 's' o 'b' desde la nube.");
}
